...
{
	int	recsock, sendsock, msgsock;  /* Sockets are integer file descriptors on Linux */
	struct	sockaddr_in	name, caller;
	char	recbuf[2048];
    char	sendbuf[2048];
	int	size, length, ret, k;

  /* Process the command line for the recbuffer size, if given */
	if (argc > 1)
	{
		size = atoi(argv[1]);
    /* Validate that the given argument is between 1 and sizeof(recbuf) - 1 
     * Set to the default size if argument is invalid */
		if (size < 1  ||  size > sizeof(recbuf) - 1) 
			size = sizeof(recbuf) - 1;
	}
	else
	{
		size = sizeof(recbuf) - 1;  /* Default size */
	}
   ...
   * Exit if the socket cannot be created */
	...
	if (recsock < 0 || sendsock < 0) 
	{
		perror("Receiver or Sending: socket() failed. ");
		return (-1);
	}
   ...
   * we can call bind(). Again, exit if bind() fails. */
	...
	ret = bind(recsock,(struct sockaddr *)&name,sizeof name);
	if (ret < 0)
	{
		perror("receiver: bind() failed. ");
		return (-1);
	}
    ret = bind(sendsock,(struct sockaddr *)&name,sizeof name);
	if (ret < 0)
	{
		perror("sending: bind() failed. ");
		return (-1);
	}
    ...
    * system assigned values and then print that info to the screen. */
	length = sizeof name;
	ret = getsockname(recsock, (struct sockaddr *)&name, (socklen_t *)&length);
	if (ret < 0)
	{
		perror("receiver: getsockname() failed. ");
		return (-1);
	}
    ret = getsockname(sendsock, (struct sockaddr *)&name, (socklen_t *)&length);
	if (ret < 0)
	{
		perror("sender: getsockname() failed. ");
		return (-1);
	}
   ...
   * accept() function will block until there is a client or an error. */
	...
	listen(recsock,5);		/* up to 5 clients can connect. only 1st is accepted */
	k = sizeof caller;
	msgsock = accept(recsock, (struct sockaddr *)&caller, (socklen_t *)&k);
   ...
   * check the value of msgsock (the data socket) to see which has happened */
    ...
    * all that remains is closing the sockets. */
	...
	close(recsock);      /* close listen socket */
